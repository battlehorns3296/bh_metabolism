...
AddEventHandler("fred:useItem", function(index)
	...
	if (Config.ItemsToUse[index]["EatOrDrink"] == "drink") then
		TaskItemInteraction(PlayerPedId(), nil, GetHashKey("DRINK_Bottle_Cylinder_d1-55_H18_Neck_A8_B1-8_QUICK_LEFT_HAND"), true, 0, 0)
	end

	if (Config.ItemsToUse[index]["Name"] == "canteen_full") then
		TriggerServerEvent('fred:drinkcanteen_full')
	elseif (Config.ItemsToUse[index]["Name"] == "canteen_80") then
		TriggerServerEvent('fred:drinkcanteen_80')
	elseif (Config.ItemsToUse[index]["Name"] == "canteen_60") then
		TriggerServerEvent('fred:drinkcanteen_60')
	elseif (Config.ItemsToUse[index]["Name"] == "canteen_40") then
		TriggerServerEvent('fred:drinkcanteen_40')
	elseif (Config.ItemsToUse[index]["Name"] == "canteen_20") then
		TriggerServerEvent('fred:drinkcanteen_20')
	end
